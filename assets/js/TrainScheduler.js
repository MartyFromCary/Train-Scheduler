...
trainRef.orderByChild("dateAdded").on(
  "child_added",
  function(snap) {
    ...
    var diffTime = timeNow.diff(
      moment(trainData.firsttime, "HH:mm"),
      "minutes"
    );
  ...
  for (let index in formDiv) {
    ...
    switch (index) {
      ...
      case "firsttime":
        ...
        if (trainDataItem == "") {
          errorMsg =
            "<strong>First Train Time</strong> is not correctly formatted.<br>Please correct and resubmit";
          $("#input-error").html(errorMsg);
          myModal.modal();
          return;
        }
      ...
      case "frequency":
        if (trainDataItem <= 0) {
          errorMsg =
            "<strong>Frequency</strong> must be greater than 0.<br>Please correct and resubmit";
          $("#input-error").html(errorMsg);
          myModal.modal();
          return;
        }
